...
export const params = {
  ...
  yaw: { value: 0.6, min: -3.14, max: 3.14, group: 'Camera' }, // → its own folder
};
...
- `group: 'Camera'` in a descriptor puts that param in a folder of its own; ungrouped params share `Params`. Group folders are built in declaration order between `Params` and `Size`, and an empty `Params` (every param grouped) is hidden — `folders.params` still exists for a plugin, which clears `hidden` itself.
